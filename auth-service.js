...
// Utilitaires pour tokens internes
const { createInternalToken } = require('./data-service/utils/internalToken');
...
// Route protégée - Auth + Rate Limit + Proof of Work + Appel à data-service
app.get('/secure-data',
  ...
  async (req, res) => {
    try {
      const { sub: userId, jti } = req.userPayload;

      // Génération du token interne pour la communication inter-microservices
      const { token: internalToken } = createInternalToken(userId, 300);

      logger.info('Token interne généré', { userId, jti });

      // Appel au data-service
      const dataServiceResponse = await fetch(`${DATA_SERVICE_URL}/internal/data`, {
        method: 'GET',
        headers: {
      ...
      if (!dataServiceResponse.ok) {
        const errorData = await dataServiceResponse.json();
        logger.warn('Data-service retourna une erreur', { 
          status: dataServiceResponse.status,
          error: errorData.error,
          userId
        });
        return res.status(dataServiceResponse.status).json({ 
          error: 'Erreur lors de l\'accès aux données',
          details: errorData.error
        });
      }

      const dataServiceData = await dataServiceResponse.json();

      logger.info('Data-service appelé avec succès', { 
        userId, 
        jti,
        dataServiceJti: dataServiceData.jti
      });
